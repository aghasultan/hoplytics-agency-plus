 ...
 */
function hoplytics_seo_audit_shortcode(): string
{
    ob_start();
    ...
    if (isset($_GET['seo_audit_submitted']) && 'true' === $_GET['seo_audit_submitted']) {
        ...
        <div class="seo-audit-wrapper card bg-alt">
            <div class="text-center">
                ...
                <p><?php _e('We received your request. Your PDF report will be emailed to you within 24 hours.', 'hoplytics'); ?>
                </p>
                <a href="<?php echo esc_url(remove_query_arg('seo_audit_submitted')); ?>"
                    class="btn btn-sm btn-link mt-4"><?php _e('Analyze Another Site', 'hoplytics'); ?></a>
            </div>
        </div>
        <?php
        return ob_get_clean();
    ...
    <div class="seo-audit-wrapper card bg-alt">
        ...
        <form action="<?php echo esc_url(admin_url('admin-post.php')); ?>" method="post"
            class="seo-audit-form flex gap-4" style="margin-top: 1.5rem; flex-wrap: wrap;">
            <input type="hidden" name="action" value="hoplytics_seo_audit">
            <?php wp_nonce_field('seo_audit_action', 'seo_audit_nonce'); ?>

            <input type="url" name="seo_url" placeholder="https://yourwebsite.com" required
                style="flex: 1; min-width: 250px; margin-bottom: 0;">
            <input type="email" name="seo_email" placeholder="Your Email" required
                style="flex: 1; min-width: 250px; margin-bottom: 0;">
            <button type="submit" class="btn btn-primary"><?php _e('Analyze Now', 'hoplytics'); ?></button>
        </form>
        <p class="text-xs text-center mt-2 text-muted">
            <?php _e('We will email your PDF report within 24 hours.', 'hoplytics'); ?></p>
    </div>
    <?php
    return ob_get_clean();
}
 ...
 */
function hoplytics_handle_seo_audit_form(): void
{
